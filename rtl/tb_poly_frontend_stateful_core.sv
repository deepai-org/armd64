...
module tb_poly_frontend_stateful_core;
  ...
  initial begin
    ...
    rst_ni = 1'b1;
    tick();
    check(state_frontend_o == POLY_FRONTEND_X86 && state_pc_o == 64'd0,
      "reset state is x86 zero");

    init_i = 1'b1;
    init_frontend_i = POLY_FRONTEND_AARCH64;
    init_pc_i = 64'h4000;
    #1;
    check(state_update_o && redirect_valid_o, "init redirects state");
    check(redirect_frontend_o == POLY_FRONTEND_AARCH64 &&
      redirect_pc_o == 64'h4000, "init redirect target");
    tick();
    init_i = 1'b0;
    #1;
    check(state_frontend_o == POLY_FRONTEND_AARCH64 && state_pc_o == 64'h4000,
      "init updates architectural state");

    valid_i = 1'b1;
    raw_mem_resp_valid_i = 1'b1;
    raw_mem_resp_word_i = 32'h52800000;
    target_frontend_i = POLY_FRONTEND_X86;
    target_pc_i = 64'h1000;
    signature_slot_valid_i = 1'b1;
    #1;
    check(raw_mem_req_valid_o && retire_o && !fault_o, "raw non-control retires");
    check(commit_frontend_o == POLY_FRONTEND_AARCH64 && commit_pc_o == 64'h4004,
      "raw non-control commit target");
    check(state_update_o && redirect_pc_o == 64'h4004, "raw fallthrough redirects state");
    tick();
    clear_inputs();
    #1;
    check(state_frontend_o == POLY_FRONTEND_AARCH64 && state_pc_o == 64'h4004,
      "raw fallthrough updates architectural state");
    ...
    check(state_update_o && redirect_frontend_o == POLY_FRONTEND_AARCH64 &&
      redirect_pc_o == 64'h400c, "raw direct branch redirects to decoded target");
    tick();
    clear_inputs();
    #1;
    ...
    clear_inputs();
    #1;
    check(state_frontend_o == POLY_FRONTEND_X86 && state_pc_o == 64'h1000,
      "reinit to x86 caller state");
    ...
    check(state_update_o && redirect_frontend_o == POLY_FRONTEND_AARCH64 &&
      redirect_pc_o == 64'h5000, "stateful pcall redirects state");
    tick();
    clear_inputs();
    #1;
    check(state_frontend_o == POLY_FRONTEND_AARCH64 && state_pc_o == 64'h5000,
      "pcall updates architectural state");
    check(transition_stack_depth_o == 4'd1, "pcall pushed transition frame");
    ...
    check(return_cookie_hit_o && return_recover_pop_o && return_resume_o,
      "stateful return cookie recovers");
    check(return_resume_frontend_o == POLY_FRONTEND_X86 &&
      return_resume_pc_o == 64'h1004 && return_resume_sp_o == 64'h8000,
      "stateful return cookie resume target");
    check(state_update_o && redirect_frontend_o == POLY_FRONTEND_X86 &&
      redirect_pc_o == 64'h1004, "return cookie redirects state");
    tick();
    clear_inputs();
    #1;
    check(state_frontend_o == POLY_FRONTEND_X86 && state_pc_o == 64'h1004,
    ...
    x86_fetch_valid_i = 1'b0;
    target_frontend_i = POLY_FRONTEND_AARCH64;
    target_pc_i = 64'h6000;
    signature_slot_valid_i = 1'b1;
    #1;
    check(wait_fetch_o && !state_update_o && state_hold_o,
      "fetch wait stalls state update");
    tick();
    clear_inputs();
    #1;
    check(state_frontend_o == POLY_FRONTEND_X86 && state_pc_o == 64'h1004,
      "fetch wait preserves architectural state");

    $display("POLY_RTL_FRONTEND_STATEFUL_CORE_SIM_OK");
    $finish;
  end
endmodule
