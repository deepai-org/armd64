...
- `poly_frontend_fpga_top.sv`: FPGA-facing top that wraps the stateful core and
  ...
  qwords, state validation/error sidebands, ABI signature programming/apply
  metadata, and transition cycle-budget diagnostics to external execute, data,
  trap-monitor, register-alias, and timing-integration paths.
...
- `poly_raw_data_mem_response_stage.sv`: raw data-memory response adapter that
  converts request validation errors and memory response faults into precise
  execute-stage resolved/fault signals for the frontend core.
...
- `test_poly_frontend_fpga_top.py`: FPGA-top wiring checks for the unified
  ...
  validation/error outputs, ABI signature sideband outputs, transition
  cycle-budget outputs, and absence of OS/runtime policy.
...
- `test_poly_raw_data_mem_response_stage.py`: raw data-memory response-stage
  checks for memory wait, clean completion, request-validation faults, and
  memory-response faults.
...
POLY_RTL_CPUID_ROM_SIM_OK
POLY_RTL_FRONTEND_RETIRE_SIM_OK
POLY_RTL_FRONTEND_PREDECODED_RETIRE_SIM_OK
POLY_RTL_FRONTEND_MEMORY_RETIRE_SIM_OK
POLY_RTL_FRONTEND_CORE_SIM_OK
POLY_RTL_FRONTEND_STATEFUL_CORE_SIM_OK
POLY_RTL_FRONTEND_FPGA_TOP_SIM_OK
POLY_RTL_FRONTEND_STATE_SIM_OK
POLY_RTL_TRANSITION_STACK_SIM_OK
POLY_RTL_ABI_SIGNATURE_SLOTS_SIM_OK
POLY_RTL_INTERRUPT_BOUNDARY_SIM_OK
POLY_RTL_RETURN_COOKIE_RECOVER_SIM_OK
POLY_RTL_TRAP_PACKET_STAGE_SIM_OK
POLY_RTL_MEMORY_ORDER_SIM_OK
```
