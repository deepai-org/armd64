...
- `poly_frontend_core.sv`: frontend core wrapper that connects fetch-to-retire
  ...
  trap-packet delivery, ABI signature lookup, and native return-cookie recovery
  to the hardware transition stack.
...
- `test_poly_frontend_core.py`: frontend/transition-stack integration checks
  ...
  signature lookup, return-cookie recovery, and return-pop conflict avoidance.
