...
- `poly_frontend_fpga_top.sv`: FPGA-facing top that wraps the stateful core and
  ...
  request/response/fault sidebands, state validation/error sidebands, and ABI
  signature programming/apply metadata to external execute, data, and
  register-alias paths.
...
- `test_poly_frontend_fpga_top.py`: FPGA-top wiring checks for the unified
  ...
  outputs, state validation/error outputs, ABI signature sideband outputs, and
