...
// recovery, and trap-packet delivery.
module poly_transition_cycle_budget (
    ...
    input  logic       monitor_packet_ready_i,
  ...
  always_comb begin
    op_supported =
      ...
      op_i == POLY_CYCLE_OP_TRAP_PACKET;
    requires_signature = op_i == POLY_CYCLE_OP_PCALL_REG;
    requires_transition_stack =
      op_i == POLY_CYCLE_OP_PCALL_REG ||
      op_i == POLY_CYCLE_OP_RETURN_COOKIE;
    requires_packet = op_i == POLY_CYCLE_OP_TRAP_PACKET;
    ...
    blocked_o =
      ...
      ((requires_signature && !signature_slot_valid_i) ||
       ...
       (requires_packet && !monitor_packet_ready_i));
    ...
    unique case (op_i)
      ...
      POLY_CYCLE_OP_TRAP_PACKET: fixed_cycles_o = 8'd2;
      default: fixed_cycles_o = 8'd0;
    endcase

    waits_for_memory_o =
      valid_i && op_i == POLY_CYCLE_OP_TRAP_PACKET && !invalid_op_o;
