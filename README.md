...
`riscv-pcall-libc-import-real.so#poly_entry`) that call `strlen`, `strcmp`, `memcpy`,
`memmove`, `memset`, and `memcmp` through PLT/GOT descriptors, compiler-built imported-object
...
`poly_import_add`, `strlen`, `strcmp`, `memcpy`, `memmove`, `memset`, and
`memcmp`, and teardown
before returning.
