...
POLY_ALPINE_PODMAN_RUN_TIMEOUT ?= 240
POLY_ALPINE_PODMAN_EXTRA_RUN_ARGS ?=
POLY_ALPINE_TRACE_PODMAN_STORAGE_PATHS ?= 0
POLY_ALPINE_TRACE_PODMAN_SYNC_FDS ?= 0
...
check-poly-abi-descriptor:
	tmp_dir=$$(mktemp -d); \
	trap 'rm -rf "$$tmp_dir"' EXIT; \
	$(CC) -std=gnu11 -Wall -Wextra -Werror \
		-Itools/include -Itools/runtime \
		tools/tests/poly_abi_descriptor_test.c \
		tools/runtime/abi/poly_abi_descriptor.c \
		-o "$$tmp_dir/poly_abi_descriptor_test"; \
	"$$tmp_dir/poly_abi_descriptor_test"
...
check-poly-contracts: check-poly-import-ids check-poly-isa-readiness check-poly-arch-contract check-poly-cpuid-contract check-poly-state-layout check-poly-abi-descriptor check-poly-abi-classify check-poly-bridge-ir
...
boot-poly-alpine-binfmt-network-smoke:
	docker run --rm \
		--platform=linux/arm64 \
		-v "$(CURDIR)":/work \
		$(BOOT_DOCKER_ENV) \
		-e BOCHS_MEGS=512 \
		-e POLY_ENABLED=1 \
		-e RUN_POLY_ALPINE_BINFMT_SMOKE=1 \
		$(IMAGE) \
		./scripts/boot.sh

boot-poly-alpine-container-smoke:
	docker run --rm \
		--platform=linux/arm64 \
		-v "$(CURDIR)":/work \
		$(BOOT_DOCKER_ENV) \
		-e BOCHS_MEGS=512 \
		-e POLY_ENABLED=1 \
		-e RUN_POLY_ALPINE_CONTAINER_SMOKE=1 \
		$(IMAGE) \
		./scripts/boot.sh

boot-poly-alpine-runc-version-smoke:
	docker run --rm \
		--platform=linux/arm64 \
		-v "$(CURDIR)":/work \
		$(BOOT_DOCKER_ENV) \
		...
		-e BOCHS_MEGS=1024 \
		-e POLY_ENABLED=1 \
		-e RUN_POLY_ALPINE_PODMAN_SMOKE=1 \
		-e POLY_ALPINE_PROTECT_RUNTIME_SIGNALS=1 \
		-e POLY_ALPINE_PODMAN_SMOKE_MODE=runc-version \
		-e POLY_ALPINE_PODMAN_RUN_TIMEOUT=$(POLY_ALPINE_PODMAN_RUN_TIMEOUT) \
		-e POLY_ALPINE_TRACE_PROCESS_LIFECYCLE=1 \
		-e POLY_ALPINE_TRACE_PODMAN_STORAGE_PATHS=$(POLY_ALPINE_TRACE_PODMAN_STORAGE_PATHS) \
		-e POLY_ALPINE_TRACE_PODMAN_SYNC_FDS=$(POLY_ALPINE_TRACE_PODMAN_SYNC_FDS) \
		-e POLY_ALPINE_POLYEXEC_SYSCALL_SUMMARY=1 \
		$(IMAGE) \
		./scripts/boot.sh

boot-poly-alpine-podman-local-smoke:
	docker run --rm \
		--platform=linux/arm64 \
		-v "$(CURDIR)":/work \
		$(BOOT_DOCKER_ENV) \
		...
		-e RUN_POLY_ALPINE_PODMAN_SMOKE=1 \
		-e POLY_ALPINE_PROTECT_RUNTIME_SIGNALS=1 \
		-e POLY_ALPINE_PODMAN_SMOKE_MODE=local \
		-e POLY_ALPINE_PODMAN_RUN_TIMEOUT=$(POLY_ALPINE_PODMAN_RUN_TIMEOUT) \
		-e POLY_ALPINE_PODMAN_EXTRA_RUN_ARGS="$(POLY_ALPINE_PODMAN_EXTRA_RUN_ARGS)" \
		-e POLY_ALPINE_TRACE_PROCESS_LIFECYCLE=1 \
		-e POLY_ALPINE_TRACE_PODMAN_STORAGE_PATHS=$(POLY_ALPINE_TRACE_PODMAN_STORAGE_PATHS) \
		-e POLY_ALPINE_TRACE_PODMAN_SYNC_FDS=$(POLY_ALPINE_TRACE_PODMAN_SYNC_FDS) \
		-e POLY_ALPINE_POLYEXEC_SYSCALL_SUMMARY=1 \
...
boot-poly-alpine-node-smoke:
	docker run --rm \
		...
		-e BOCHS_MEGS=1536 \
		-e POLY_ENABLED=1 \
		-e RUN_POLY_ALPINE_NODE_SMOKE=1 \
		$(IMAGE) \
		./scripts/boot.sh

boot-poly-alpine-postgres-smoke:
	docker run --rm \
		--platform=linux/arm64 \
		-v "$(CURDIR)":/work \
		$(BOOT_DOCKER_ENV) \
...
boot-poly-alpine-fio-io-uring:
	docker run --rm \
		--platform=linux/arm64 \
		-v "$(CURDIR)":/work \
		$(BOOT_DOCKER_ENV) \
		-e BOOT_TIMEOUT_SECONDS=1200 \
		-e BOCHS_MEGS=768 \
		-e POLY_ENABLED=1 \
...
boot-poly-alpine-sqlite-stress:
	docker run --rm \
		--platform=linux/arm64 \
		-v "$(CURDIR)":/work \
		$(BOOT_DOCKER_ENV) \
		...
		-e BOCHS_MEGS=768 \
		-e POLY_ENABLED=1 \
		-e RUN_POLY_ALPINE_SQLITE_STRESS=1 \
		$(IMAGE) \
		./scripts/boot.sh
