...
build_poly_elf_payloads() {
  ...
  "$POLY_ELF_GEN_BIN" aarch64 "$TMP_DIR/initramfs-root/usr/lib/polyapps/aarch64-pcall-dynrel.elf" --dyn-relative64 0x7b 0xd0000000 0x91000000 0xf9400000 0xf9400000 0xd65f03c0
  ...
  "$POLY_ELF_GEN_BIN" riscv "$TMP_DIR/initramfs-root/usr/lib/polyapps/riscv-pcall-dynrel.elf" --dyn-relative64 0x7b 0x00002517 0x00053503 0x00053503 0x00008067
...
if [ "$RUN_POLY_APPS" = "1" ]; then
  /usr/bin/polyapp /usr/lib/polyapps/*.poly >/dev/ttyS0 2>&1
    ...
    /usr/lib/polyapps/aarch64-pcall-dynrel.elf=123 \
    /usr/lib/polyapps/riscv-pcall-frame.elf=45 \
    /usr/lib/polyapps/riscv-pcall-split-load.elf=123 \
    /usr/lib/polyapps/riscv-pcall-dynrel.elf=123 >/dev/ttyS0 2>&1
fi
