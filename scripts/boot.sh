  ...
  while true; do
    ...
    if grep -q "BOOT_OK: initramfs reached userspace" "$SERIAL_LOG"; then
      ...
      if [[ "$RUN_POLY_CALL" == "1" ]]; then
        ...
        if ! grep -Eq "POLYCALL_ROOT_PCALL: arch=aarch64 exchange_u64=0 call_kind=23 .*special_sig=1 .*sret_thunk=0 .*fp64_stack_thunk=0 .*hfa_f64_arg_thunk=0 .*hfa_f64_ret_thunk=0 .*hfa_f32_ret_thunk=0 .*aarch64-pcall-hfa3-real\\.so" "$SERIAL_LOG"; then
          sleep 1
          continue
        fi
        if ! grep -Eq "POLYCALL_ROOT_PCALL: arch=aarch64 exchange_u64=0 call_kind=24 .*special_sig=1 .*sret_thunk=0 .*fp64_stack_thunk=0 .*hfa_f64_arg_thunk=0 .*hfa_f64_ret_thunk=0 .*hfa_f32_ret_thunk=0 .*aarch64-pcall-hfa4-real\\.so" "$SERIAL_LOG"; then
          sleep 1
          continue
        fi
