...
uint64_t POLY_HOST_HELPER poly_host_x86_sum14(uint64_t a, uint64_t b,
    uint64_t c, uint64_t d, uint64_t e, uint64_t f, uint64_t g, uint64_t h,
    uint64_t i, uint64_t j, uint64_t k, uint64_t l, uint64_t m, uint64_t n)
{
  const uint64_t result = poly_host_x86_add_bias(a + b + c + d + e + f + g +
    h + i + j + k + l + m + n);
  return result + poly_host_x86_zero;
}
...
__asm__(
  ...
  "  movq %r10, -8(%rsp)\n"
  "  movabsq $0x8877665544332211, %r11\n"
  "  movq %r11, -128(%rsp)\n"
  "  movq -8(%rsp), %rax\n"
  "  cmpq %r10, %rax\n"
  "  jne 1f\n"
...
double POLY_HOST_HELPER poly_host_x86_fp64_sum10(double a, double b, double c,
...
struct poly_host_sret_u64 POLY_HOST_HELPER poly_host_x86_sret_u64_stack(
    uint64_t a, uint64_t b, uint64_t c, uint64_t d, uint64_t e, uint64_t f)
{
  (void) d;
  (void) e;
  struct poly_host_sret_u64 result = { a + 10, b + 20, c + 30, f + 60 };
  return result;
}
...
uint64_t POLY_HOST_HELPER poly_host_x86_qsort_r(void *base, uint64_t nmemb,
    uint64_t size, void *compar, void *arg)
{
  ...
  for (uint64_t end = nmemb; end > 1; end--) {
    for (uint64_t index = 1; index < end; index++) {
      uint8_t *left = items + (index - 1) * size;
      uint8_t *right = items + index * size;
      if (poly_runtime_call_foreign_comparator_arg(
            compar, left, right, arg) > 0)
        poly_host_swap_records(left, right, size);
    }
  }
...
double POLY_HOST_HELPER poly_host_x86_fabs(double value)
{
  union {
    double d;
    uint64_t u;
  } bits = { value };
  bits.u &= 0x7fffffffffffffffULL;
  return bits.d;
}
...
uint64_t POLY_HOST_HELPER poly_host_x86_ffs(uint64_t value)
{
...
uint64_t POLY_HOST_HELPER poly_host_x86_ffsl(uint64_t value)
{
  if (value == 0)
    return 0;
...
uint64_t POLY_HOST_HELPER poly_host_x86_ffsll(uint64_t value)
{
  return poly_host_x86_ffsl(value);
}
