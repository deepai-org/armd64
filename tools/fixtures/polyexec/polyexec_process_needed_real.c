...
struct poly_hfa3_f64 {
  double a;
  double b;
  double c;
};

struct poly_hfa4_f64 {
  double a;
  double b;
  double c;
  double d;
...
static uint64_t pack_poly_f64_hi16(double value) {
  union poly_double_bits bits;
  bits.d = value;
  return bits.u >> 48;
}
...
#elif defined(POLY_PROCESS_NEEDED_HFA64_DEP)
...
__asm__(
  ...
  ".asciz \"POLYABI\"\n"
  ".balign 4\n"
  "1: .ascii \"poly_process_needed_hfa3_f64 aarch64_hfa3_f64_ret\\n\"\n"
  "   .ascii \"poly_process_needed_hfa4_f64 aarch64_hfa4_f64_ret\\n\"\n"
  "2:\n"
  ".balign 4\n"
  ".previous\n");

__attribute__((visibility("default")))
struct poly_hfa3_f64 poly_process_needed_hfa3_f64(double a, double b,
    double c) {
  struct poly_hfa3_f64 out;
  out.a = a + b;
  out.b = b + c;
  out.c = a + c + 1.0;
  return out;
...
__attribute__((visibility("default")))
struct poly_hfa4_f64 poly_process_needed_hfa4_f64(double a, double b,
    double c) {
  struct poly_hfa4_f64 out;
  out.a = a + b;
  out.b = b + c;
  out.c = a + c + 1.0;
  out.d = a + b + c + 2.0;
...
#elif defined(POLY_PROCESS_NEEDED_HFA32_ARG_DEP)
...
__asm__(
  ...
  ".asciz \"POLYABI\"\n"
  ".balign 4\n"
  "1: .ascii \"poly_process_needed_hfa3_f32_arg aarch64_hfa3_f32_arg\\n\"\n"
  "   .ascii \"poly_process_needed_hfa4_f32_arg aarch64_hfa4_f32_arg\\n\"\n"
  "2:\n"
  ".balign 4\n"
  ".previous\n");

__attribute__((visibility("default")))
float poly_process_needed_hfa3_f32_arg(struct poly_hfa3_f32 arg,
    float scale) {
  return (arg.a + arg.b + arg.c) * scale;
}

__attribute__((visibility("default")))
float poly_process_needed_hfa4_f32_arg(struct poly_hfa4_f32 arg,
    float scale) {
...
__asm__(
  ...
  ".asciz \"POLYABI\"\n"
  ".balign 4\n"
  "1: .ascii \"poly_process_needed_fpair32_arg fpair32_arg\\n\"\n"
  "   .ascii \"poly_process_needed_fpair64_arg fpair64_arg\\n\"\n"
  "2:\n"
  ".balign 4\n"
  ".previous\n");

__attribute__((visibility("default")))
float poly_process_needed_fpair32_arg(struct poly_fpair32 pair, float scale) {
  return (pair.lo + pair.hi) * scale;
}

__attribute__((visibility("default")))
double poly_process_needed_fpair64_arg(struct poly_fpair64 pair, double scale) {
  return (pair.lo + pair.hi) * scale;
}

#elif defined(POLY_PROCESS_NEEDED_FPAIR_RET_DEP)
...
__asm__(
  ...
  ".asciz \"POLYABI\"\n"
  ".balign 4\n"
  "1: .ascii \"poly_process_needed_fpair32_ret fpair32_ret\\n\"\n"
  "   .ascii \"poly_process_needed_fpair64_ret fpair64_ret\\n\"\n"
  "2:\n"
  ".balign 4\n"
  ".previous\n");

__attribute__((visibility("default")))
struct poly_fpair32 poly_process_needed_fpair32_ret(float a, float b,
    float c) {
  struct poly_fpair32 out;
  out.lo = a + b;
  out.hi = b + c + 1.0f;
  return out;
}
...
__attribute__((visibility("default")))
struct poly_fpair64 poly_process_needed_fpair64_ret(double a, double b,
    double c) {
...
#elif defined(POLY_PROCESS_NEEDED_HFA64_MAIN)
extern struct poly_hfa3_f64 poly_process_needed_hfa3_f64(double, double,
    double);
extern struct poly_hfa4_f64 poly_process_needed_hfa4_f64(double, double,
    double);
...
uint64_t poly_process_main(void) {
...
#elif defined(POLY_PROCESS_NEEDED_HFA64_MAIN)
  const struct poly_hfa3_f64 hfa3 =
    poly_process_needed_hfa3_f64(1.5, 2.25, 3.0);
  const struct poly_hfa4_f64 hfa4 =
    poly_process_needed_hfa4_f64(1.5, 2.25, 3.0);
  const uint64_t packed_hfa3 =
    (pack_poly_f64_hi16(hfa3.a) << 32) |
    (pack_poly_f64_hi16(hfa3.b) << 16) |
    pack_poly_f64_hi16(hfa3.c);
  const uint64_t packed_hfa4 =
    (pack_poly_f64_hi16(hfa4.a) << 48) |
    (pack_poly_f64_hi16(hfa4.b) << 32) |
    (pack_poly_f64_hi16(hfa4.c) << 16) |
    pack_poly_f64_hi16(hfa4.d);
  if (packed_hfa3 != 0x0000400e40154016ULL ||
      packed_hfa4 != 0x400e401540164021ULL)
    return 57;
  static const char marker[] = "POLY_PROCESS_CROSS_HFA64_NEEDED_OK\n";
#elif defined(POLY_PROCESS_NEEDED_HFA32_ARG_MAIN)
  const struct poly_hfa3_f32 hfa3 = { 1.5f, 2.25f, 3.0f };
  const struct poly_hfa4_f32 hfa4 = { 1.5f, 2.25f, 3.0f, 4.0f };
  const union poly_float_bits hfa3_result = {
    .f = poly_process_needed_hfa3_f32_arg(hfa3, 6.0f)
  };
  const union poly_float_bits hfa4_result = {
    .f = poly_process_needed_hfa4_f32_arg(hfa4, 6.0f)
  };
  ...
  const union poly_float_bits f32_result = {
    .f = poly_process_needed_fpair32_arg(f32_pair, 6.0f)
  };
  ...
  const struct poly_fpair32 f32_pair =
    poly_process_needed_fpair32_ret(1.5f, 2.25f, 3.0f);
  const struct poly_fpair64 f64_pair =
    poly_process_needed_fpair64_ret(1.5, 2.25, 3.0);
  const union poly_float_bits f32_lo = { .f = f32_pair.lo };
  const union poly_float_bits f32_hi = { .f = f32_pair.hi };
  const union poly_double_bits f64_lo = { .d = f64_pair.lo };
  const union poly_double_bits f64_hi = { .d = f64_pair.hi };
  if (f32_lo.u != 0x40700000U || f32_hi.u != 0x40c80000U ||
      f64_lo.u != 0x400e000000000000ULL ||
