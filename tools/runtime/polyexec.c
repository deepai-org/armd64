...
static __thread volatile sig_atomic_t poly_runtime_signal_signo;
static __thread volatile sig_atomic_t poly_runtime_signal_code;
static __thread volatile uintptr_t poly_runtime_signal_addr;
static __thread volatile sig_atomic_t poly_fatal_debug_note_written;
...
static int refresh_poly_trap_event_frame(void) {
  memset(&poly_runtime_event_frame, 0, sizeof(poly_runtime_event_frame));
  return poly_event_ptr_set(
    (uint64_t) (uintptr_t) &poly_runtime_event_frame,
...
static inline uint64_t poly_dump_state_value(struct poly_v2_debug_note *note,
...
static int poly_append_char(char *path, size_t path_size, size_t *pos,
    char value) {
  if (*pos + 1U >= path_size)
    return -1;
  path[*pos] = value;
...
static int poly_append_cstr(char *path, size_t path_size, size_t *pos,
    const char *text) {
  if (text == NULL)
    return -1;
  for (size_t n = 0; text[n] != '\0'; n++) {
    if (poly_append_char(path, path_size, pos, text[n]) < 0)
      return -1;
  }
  return 0;
}

static int poly_append_u64_dec(char *path, size_t path_size, size_t *pos,
    uint64_t value) {
  char digits[20];
  size_t used = 0;
  do {
  ...
  while (used != 0) {
    if (poly_append_char(path, path_size, pos, digits[--used]) < 0)
      return -1;
  }
  return 0;
}
...
static int poly_build_fatal_debug_note_path(char *path, size_t path_size,
  ...
  if (poly_append_cstr(path, path_size, &pos,
        polyexec_fatal_debug_note_dir) < 0)
    return -1;
  if (pos != 0 && path[pos - 1U] != '/' &&
      poly_append_char(path, path_size, &pos, '/') < 0)
    return -1;
  if (poly_append_cstr(path, path_size, &pos,
        "polyexec-v2-fatal-note-") < 0 ||
      poly_append_u64_dec(path, path_size, &pos, (uint64_t) getpid()) < 0 ||
      poly_append_char(path, path_size, &pos, '-') < 0 ||
      poly_append_u64_dec(path, path_size, &pos,
...
static int poly_write_all_fd(int fd, const void *data, size_t bytes) {
...
static uint16_t poly_debug_note_elf_machine(uint32_t frontend) {
  if (frontend == POLY_MODE_RAW_AARCH64)
    return EM_AARCH64;
  if (frontend == POLY_MODE_RAW_RISCV)
    return EM_RISCV;
  return EM_X86_64;
}
...
static int poly_write_fatal_debug_note_elf(const char *path,
  ...
  ehdr.e_type = ET_CORE;
  ehdr.e_machine = poly_debug_note_elf_machine(note->frontend);
...
static void poly_write_fatal_debug_note(int signum, const char *reason) {
  if (polyexec_fatal_debug_note_dir == NULL ||
      polyexec_fatal_debug_note_dir[0] == '\0')
    return;
  if (poly_fatal_debug_note_written)
    return;
  poly_fatal_debug_note_written = 1;
  memset(&poly_fatal_debug_note, 0, sizeof(poly_fatal_debug_note));
  uint64_t result = poly_dump_state_value(&poly_fatal_debug_note,
    sizeof(poly_fatal_debug_note), POLY_V2_DUMP_SELECTOR_LIVE);
  if (result != 0 ||
  ...
  poly_write_literal_stderr("POLYEXEC_FATAL_DEBUG_NOTE: path=");
  poly_write_literal_stderr(path);
  poly_write_literal_stderr(" reason=");
  poly_write_literal_stderr(reason != NULL ? reason : "unknown");
  poly_write_literal_stderr(" selector=0x");
...
static void poly_runtime_signal_handler(int signo, siginfo_t *info,
    void *ucontext) {
  poly_runtime_signal_signo = signo;
  poly_runtime_signal_code = info != NULL ? info->si_code : 0;
  poly_runtime_signal_addr =
  ...
  poly_write_fatal_debug_note(signo, "unhandled-signal");
  _exit(128 + signo);
}
...
static int install_poly_signal_diagnostics(void) {
  if (!poly_runtime_signal_alt_stack_installed) {
    ...
    if (sigaltstack(&stack, NULL) != 0) {
      fprintf(stderr,
        "POLYEXEC_FAIL: signal diagnostic altstack install failed: %s\n",
        strerror(errno));
      return -1;
    }
  ...
  struct sigaction action;
  memset(&action, 0, sizeof(action));
  action.sa_sigaction = poly_runtime_signal_handler;
  ...
  if (sigaction(SIGSEGV, &action, NULL) != 0 ||
    ...
    fprintf(stderr,
      "POLYEXEC_FAIL: signal diagnostic handler install failed: %s\n",
      strerror(errno));
    return -1;
  }
  return 0;
...
static void clear_poly_event_frame(void) {
  (void) poly_event_ptr_set(0, 0);
}

__attribute__((noreturn))
static void poly_child_exit_now(int code) {
  clear_poly_trap_vector();
  clear_poly_event_frame();
  _exit(code);
}
...
static int poly_prepare_clone_child_handoff(
  ...
  uint64_t child_frontend_stack = foreign_stack;
  if (child_frontend_stack == 0) {
    if (packet->mode == POLY_MODE_RAW_AARCH64) {
      if ((trap_state->trap_restore.flags & POLY_TRAP_RESTORE_FLAG_VALID) != 0 &&
          trap_state->trap_restore.aarch64_gpr[31] != 0)
        child_frontend_stack = trap_state->trap_restore.aarch64_gpr[31];
      else
        child_frontend_stack = trap_state->aarch64_gpr[31];
    }
    else if (packet->mode == POLY_MODE_RAW_RISCV) {
      if ((trap_state->trap_restore.flags & POLY_TRAP_RESTORE_FLAG_VALID) != 0 &&
          trap_state->trap_restore.riscv_gpr[2] != 0)
        child_frontend_stack = trap_state->trap_restore.riscv_gpr[2];
      else
        child_frontend_stack = trap_state->riscv_gpr[2];
    }
  }
  struct poly_v2_derive_descriptor desc
    __attribute__((aligned(POLY_V2_DERIVE_DESC_ALIGN)));
  ...
  desc.child_sp = child_frontend_stack;
...
static void poly_clone_child_entry(struct poly_clone_child_handoff *handoff) {
  ...
  const uint64_t frontend = handoff->state.header.current_mode;
  struct poly_v2_derive_descriptor desc
    __attribute__((aligned(POLY_V2_DERIVE_DESC_ALIGN)));
  poly_init_derive_descriptor(&desc, frontend);
  desc.flags = POLY_V2_DERIVE_FLAG_ACTIVATE_DST;
  if (poly_derive_state_value(&handoff->state, &handoff->state, &desc) != 0) {
    poly_write_literal_stderr(
      ...
      args[4] = foreign_clone_tls;
      {
        ...
        if (poly_trace_syscalls_enabled() ||
          ...
          poly_lifecycle_printf(
            ...
            (unsigned long long) clone_handoff->state.aarch64_gpr[31],
...
static int run_poly_thread_stress(const struct poly_request *request,
  ...
  for (size_t n = 0; n < created; n++) {
    printf("POLYEXEC_THREAD_RESULT: index=%zu value=%llu path=%s\n",
      n, (unsigned long long) contexts[n].result, request->path);
  }
...
int main(int argc, char **argv) {
  ...
  if (fatal_debug_note_env != NULL && fatal_debug_note_env[0] != '\0' &&
    ...
    polyexec_fatal_debug_note_dir = fatal_debug_note_env;
