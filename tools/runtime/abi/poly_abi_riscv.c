...
static struct poly_abi_loc riscv_stack_loc(struct riscv_cursor *cursor,
...
static int riscv_add_arg_loc(struct poly_abi_layout *out, size_t arg_index,
    struct riscv_cursor *cursor, enum poly_abi_loc_kind kind, uint16_t size) {
  ...
  if (kind == POLY_ABI_LOC_FPR && cursor->fpr < 8)
    return poly_abi_layout_add_arg_loc(out, arg_index,
      poly_abi_loc_reg(kind, cursor->fpr++, size));
  return poly_abi_layout_add_arg_loc(out, arg_index,
    riscv_stack_loc(cursor, size, size >= 16 ? 16 : 8));
}

static int riscv_classify_arg(const struct poly_abi_type *type,
    struct poly_abi_layout *out, size_t arg_index,
    struct riscv_cursor *cursor) {
  if (is_integer_kind(type->kind) || type->kind == POLY_ABI_TYPE_VEC128)
    return riscv_add_arg_loc(out, arg_index, cursor, POLY_ABI_LOC_GPR,
      type->size);
  if (is_float_kind(type->kind))
    return riscv_add_arg_loc(out, arg_index, cursor, POLY_ABI_LOC_FPR,
      type->size);
  if (type->kind == POLY_ABI_TYPE_HFA) {
    ...
    for (uint8_t n = 0; n < type->count; n++) {
      if (riscv_add_arg_loc(out, arg_index, cursor, POLY_ABI_LOC_FPR,
            elem_size) != 0)
        return -1;
    }
    return 0;
  ...
  if (type->kind == POLY_ABI_TYPE_AGGREGATE && type->size <= 16) {
    ...
    for (uint8_t n = 0; n < parts; n++) {
      ...
      if (riscv_add_arg_loc(out, arg_index, cursor, POLY_ABI_LOC_GPR,
            part_size) != 0)
        return -1;
    }
    return 0;
  ...
  return poly_abi_layout_add_arg_loc(out, arg_index,
    riscv_stack_loc(cursor, type->size, type->align));
}

static int riscv_classify_ret(const struct poly_abi_type *type,
...
int poly_classify_riscv_lp64d_call_abi(const struct poly_function_sig *sig,
  ...
  if (sig == 0 || out == 0 || sig->arg_count > POLY_FUNCTION_SIG_MAX_ARGS)
    return -1;
  poly_abi_layout_init(out, POLY_ABI_ARCH_RISCV);
  struct riscv_cursor cursor = { 0 };
  if ((sig->flags & POLY_FUNCTION_SIG_FLAG_RET_BY_SRET) != 0) {
    ...
    if (poly_abi_layout_add_ret_loc(out, poly_abi_loc_sret_ptr(0,
          sig->ret.size)) != 0)
      return -1;
  } else if (riscv_classify_ret(&sig->ret, out) != 0) {
    return -1;
  }
  for (size_t n = 0; n < sig->arg_count; n++) {
    if (riscv_classify_arg(&sig->args[n], out, n, &cursor) != 0)
      return -1;
  }
