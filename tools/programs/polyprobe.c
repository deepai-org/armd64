...
static inline uint64_t poly_foreign_import_count_status(void) {
  uint64_t rax;
  asm volatile(POLY_X86_CTRL_FOREIGN_IMPORT_COUNT_STATUS_ASM : "=a"(rax) ::
      "memory");
  return rax;
}
...
int main(void) {
  ...
  if (aarch64_foreign_control_plane_probe(aarch64_vector) !=
      aarch64_vector ||
  ...
  if (riscv_foreign_control_plane_probe(riscv_vector) !=
      riscv_vector ||
