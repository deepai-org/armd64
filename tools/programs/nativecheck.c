...
#define POLY_OP_MONITOR_PACKET_SET POLY_X86_CTRL_ASM_BYTE(0x6b)
#define POLY_OP_MONITOR_PACKET_GET POLY_X86_CTRL_ASM_BYTE(0x6c)
...
#define NATIVECHECK_V2_EVENT_MAGIC \
...
static int nativecheck_is_raw_foreign_mode(uint64_t mode) {
  return mode == POLY_MODE_RAW_AARCH64 || mode == POLY_MODE_RAW_RISCV;
}

static int nativecheck_event_reason(uint16_t event_kind, uint64_t *reason) {
  switch (event_kind) {
...
static int nativecheck_event_frame_to_packet(
    const struct poly_v2_event_frame *event,
    struct nativecheck_event_packet *packet) {
  uint64_t reason = 0;
  if (event == 0 || packet == 0 ||
      event->magic != NATIVECHECK_V2_EVENT_MAGIC ||
      ...
      event->insn_pc == 0 ||
      event->resume_pc == 0 ||
      !nativecheck_is_raw_foreign_mode(event->frontend) ||
      nativecheck_event_reason(event->event_kind, &reason) != 0)
    return -1;
...
static int expect_event_packet_args(const char *label,
  ...
  struct nativecheck_event_packet packet;
  if (nativecheck_event_frame_to_packet(event, &packet) != 0 ||
      packet.reason != reason ||
      packet.mode != source_mode ||
      packet.number != number ||
      packet.selector != selector) {
    fprintf(stderr,
      "NATIVE_CHECK_FAIL: poly v2 event frame %s header mismatch kind=%u mode=%u number=%llu selector=%llu pc=0x%llx resume=0x%llx sequence=%llu\n",
  ...
  for (unsigned i = 0; i < POLY_V2_EVENT_ARG_COUNT; ++i) {
    if (packet.args[i] != expected_args[i]) {
      fprintf(stderr,
        "NATIVE_CHECK_FAIL: poly v2 event frame %s arg%u mismatch got=%llu expected=%llu\n",
...
static int expect_event_packet_pc(const char *label,
    const struct poly_v2_event_frame *event, uint64_t trap_pc,
    uint64_t resume_pc) {
  struct nativecheck_event_packet packet;
  if (nativecheck_event_frame_to_packet(event, &packet) != 0 ||
      packet.pc != trap_pc ||
      packet.resume_pc != resume_pc) {
    fprintf(stderr,
      "NATIVE_CHECK_FAIL: poly v2 event frame %s pc mismatch trap=0x%llx/0x%llx resume=0x%llx/0x%llx\n",
      label,
      (unsigned long long) (event != 0 ? event->insn_pc : 0),
...
static int expect_event_packet_import_pc(const char *label,
    const struct poly_v2_event_frame *event, uint32_t import_id) {
  struct nativecheck_event_packet packet;
  const uint64_t trap_pc = POLY_IMPORT_CALL_BASE +
    (uint64_t) import_id * POLY_IMPORT_CALL_STRIDE;
  if (nativecheck_event_frame_to_packet(event, &packet) != 0 ||
      packet.pc != trap_pc || packet.resume_pc == 0 ||
      packet.resume_pc == trap_pc ||
      !nativecheck_is_canonical48(packet.resume_pc)) {
    fprintf(stderr,
      "NATIVE_CHECK_FAIL: poly v2 event frame %s import pc mismatch trap=0x%llx/0x%llx resume=0x%llx\n",
      label,
      (unsigned long long) (event != 0 ? event->insn_pc : 0),
      (unsigned long long) trap_pc,
      (unsigned long long) (event != 0 ? event->resume_pc : 0));
...
__attribute__((noreturn, noinline))
static void child_expect_aarch64_invalid_generic_call_return_signal(void) {
  poly_trap_vector_set_value(0);
  poly_trap_vector_mode_set_value(POLY_MODE_X86);
  asm volatile(
    POLY_OP_ENTER_A64
    ".long 0x10000090\n" // adr x16,target
    ".long 0xd2800051\n" // movz x17,#2 (RISC-V frontend)
    ".long 0xaa0003f2\n" // mov x18,x0 (non-canonical return from RAX)
    ".long 0xd5032f3f\n" // aarch64 generic pcall frontend=x17 target=x16
...
__attribute__((noreturn, noinline))
static void child_expect_aarch64_invalid_generic_signature_target_signal(void) {
  poly_trap_vector_set_value(0);
  poly_trap_vector_mode_set_value(POLY_MODE_X86);
  asm volatile(
    POLY_OP_ENTER_A64
...
__attribute__((noreturn, noinline))
static void child_expect_aarch64_invalid_generic_signature_return_signal(void) {
  poly_trap_vector_set_value(0);
  poly_trap_vector_mode_set_value(POLY_MODE_X86);
  asm volatile(
    POLY_OP_ENTER_A64
...
__attribute__((noreturn, noinline))
static void child_expect_aarch64_invalid_immediate_signature_target_signal(void) {
  poly_trap_vector_set_value(0);
  poly_trap_vector_mode_set_value(POLY_MODE_X86);
  asm volatile(
    POLY_OP_ENTER_A64
    ...
    ".long 0xd2800051\n" // movz x17,#2 (RISC-V frontend)
    ".long 0xd2800012\n" // movz x18,#0
    ".long 0xd5032a1f\n" // aarch64 signature pcall, immediate slot 0
    ".long 0xd5032e1f\n" // aarch64 polyctrl x86 escape
    :
    : "a"(NATIVECHECK_NONCANONICAL_ADDR)
    : "rbx", "rcx", "rdx", "rsi", "rdi",
...
__attribute__((noreturn, noinline))
static void child_expect_aarch64_invalid_immediate_signature_return_signal(void) {
  poly_trap_vector_set_value(0);
  poly_trap_vector_mode_set_value(POLY_MODE_X86);
  asm volatile(
    POLY_OP_ENTER_A64
...
__attribute__((noreturn, noinline))
static void child_expect_riscv_invalid_generic_switch_target_signal(void) {
  poly_trap_vector_set_value(0);
  poly_trap_vector_mode_set_value(POLY_MODE_X86);
  asm volatile(
    POLY_OP_ENTER_RV64
...
__attribute__((noreturn, noinline))
static void child_expect_riscv_invalid_generic_call_return_signal(void) {
  poly_trap_vector_set_value(0);
  poly_trap_vector_mode_set_value(POLY_MODE_X86);
  asm volatile(
    POLY_OP_ENTER_RV64
    ...
    ".long 0x01428293\n" // addi t0,t0,20
    ".long 0x00100313\n" // addi x6,zero,1 (AArch64 frontend)
    ".long 0x00050393\n" // addi x7,a0,0 (non-canonical return from RAX)
    ".long 0x1200700b\n" // riscv generic pcall frontend=x6 target=x5
    ".long 0xd28005a0\n" // target: movz x0,#45
    ".long 0xd65f03c0\n" // ret x30
    ".long 0x0000700b\n" // riscv polyctrl x86 escape
...
__attribute__((noreturn, noinline))
static void child_expect_riscv_invalid_generic_signature_target_signal(void) {
  poly_trap_vector_set_value(0);
  poly_trap_vector_mode_set_value(POLY_MODE_X86);
  asm volatile(
    POLY_OP_ENTER_RV64
    ...
    ".long 0x00100313\n" // addi x6,zero,1 (AArch64 frontend)
    ".long 0x00000393\n" // addi x7,zero,0
    ".long 0x00000e13\n" // addi x28,zero,0 (signature slot)
    ".long 0x1400700b\n" // riscv generic signature pcall
    ".long 0x0000700b\n" // riscv polyctrl x86 escape
    :
    : "a"(NATIVECHECK_NONCANONICAL_ADDR)
...
__attribute__((noreturn, noinline))
static void child_expect_riscv_invalid_generic_signature_return_signal(void) {
  poly_trap_vector_set_value(0);
  poly_trap_vector_mode_set_value(POLY_MODE_X86);
  asm volatile(
    POLY_OP_ENTER_RV64
...
__attribute__((noreturn, noinline))
static void child_expect_riscv_invalid_immediate_signature_target_signal(void) {
  poly_trap_vector_set_value(0);
  poly_trap_vector_mode_set_value(POLY_MODE_X86);
  asm volatile(
    POLY_OP_ENTER_RV64
    ...
    ".long 0x00100313\n" // addi x6,zero,1 (AArch64 frontend)
    ".long 0x00000393\n" // addi x7,zero,0
    ".long 0x4000700b\n" // riscv signature pcall, immediate slot 0
    ".long 0x0000700b\n" // riscv polyctrl x86 escape
    :
    : "a"(NATIVECHECK_NONCANONICAL_ADDR)
    : "rbx", "rcx", "rdx", "rsi", "rdi",
...
__attribute__((noreturn, noinline))
static void child_expect_riscv_invalid_immediate_signature_return_signal(void) {
  poly_trap_vector_set_value(0);
  poly_trap_vector_mode_set_value(POLY_MODE_X86);
  asm volatile(
    POLY_OP_ENTER_RV64
...
static int run_poly_invalid_generic_control_signal_probe(void) {
  ...
  if (expect_child_signal("poly aarch64 invalid generic pcall", SIGILL,
    ...
    return 1;
  if (expect_child_signal("poly aarch64 invalid generic pcall return", SIGILL,
        child_expect_aarch64_invalid_generic_call_return_signal) != 0)
    return 1;
  if (expect_child_signal("poly aarch64 invalid generic signature slot",
    ...
    return 1;
  if (expect_child_signal("poly aarch64 invalid generic signature target",
        SIGILL, child_expect_aarch64_invalid_generic_signature_target_signal) != 0)
    return 1;
  if (expect_child_signal("poly aarch64 invalid generic signature return",
        SIGILL, child_expect_aarch64_invalid_generic_signature_return_signal) != 0)
    return 1;
  if (expect_child_signal("poly aarch64 invalid immediate signature target",
        SIGILL, child_expect_aarch64_invalid_immediate_signature_target_signal) != 0)
    return 1;
  if (expect_child_signal("poly aarch64 invalid immediate signature return",
        SIGILL, child_expect_aarch64_invalid_immediate_signature_return_signal) != 0)
    return 1;
  ...
  if (expect_child_signal("poly riscv invalid generic pcall", SIGILL,
    ...
    return 1;
  if (expect_child_signal("poly riscv invalid generic pcall return", SIGILL,
        child_expect_riscv_invalid_generic_call_return_signal) != 0)
    return 1;
  if (expect_child_signal("poly riscv invalid generic signature slot", SIGILL,
    ...
    return 1;
  if (expect_child_signal("poly riscv invalid generic signature target", SIGILL,
        child_expect_riscv_invalid_generic_signature_target_signal) != 0)
    return 1;
  if (expect_child_signal("poly riscv invalid generic signature return", SIGILL,
        child_expect_riscv_invalid_generic_signature_return_signal) != 0)
    return 1;
  if (expect_child_signal("poly riscv invalid immediate signature target", SIGILL,
        child_expect_riscv_invalid_immediate_signature_target_signal) != 0)
    return 1;
  if (expect_child_signal("poly riscv invalid immediate signature return", SIGILL,
        child_expect_riscv_invalid_immediate_signature_return_signal) != 0)
    return 1;
...
static int run_poly_trap_vector_probe(void) {
  ...
  memset(&event_frame, 0xa5, sizeof(event_frame));
  ...
  if (expect_event_packet_args("aarch64 syscall", &event_frame,
    ...
    return 1;
  if (expect_event_packet_pc("aarch64 syscall", &event_frame,
  ...
  if (expect_event_packet_args("riscv syscall", &event_frame,
    ...
    return 1;
  if (expect_event_packet_pc("riscv syscall", &event_frame,
  ...
  if (expect_event_packet_args("riscv wide syscall",
      &event_frame, POLY_TRAP_SYSCALL, POLY_MODE_RAW_RISCV,
  ...
  if (expect_event_packet_args("riscv compressed break", &event_frame,
      POLY_TRAP_BREAK, POLY_MODE_RAW_RISCV, 5, 0,
      riscv_break_args) != 0)
    return 1;
  ...
  if (expect_event_packet_args("aarch64 import", &event_frame,
      POLY_TRAP_IMPORT, POLY_MODE_RAW_AARCH64, 8, 0,
      import_args_77_99) != 0)
    return 1;
  if (expect_event_packet_import_pc("aarch64 import", &event_frame,
  ...
  if (expect_event_packet_args("aarch64 thunk-table import",
      &event_frame, POLY_TRAP_IMPORT, POLY_MODE_RAW_AARCH64, 8, 0,
      import_args_77_99) != 0)
    return 1;
  if (expect_event_packet_import_pc("aarch64 thunk-table import",
      &event_frame, POLY_IMPORT_FUNC_STRLEN) != 0)
    return 1;
  ...
  if (expect_event_packet_args("riscv import", &event_frame,
      POLY_TRAP_IMPORT, POLY_MODE_RAW_RISCV, 8, 0,
      import_args_77_99) != 0)
    return 1;
  if (expect_event_packet_import_pc("riscv import", &event_frame,
      POLY_IMPORT_FUNC_STRLEN) != 0)
    return 1;
  ...
  if (expect_event_packet_args("riscv thunk-table import",
      &event_frame, POLY_TRAP_IMPORT, POLY_MODE_RAW_RISCV, 8, 0,
      import_args_77_99) != 0)
    return 1;
  if (expect_event_packet_import_pc("riscv thunk-table import",
      &event_frame, POLY_IMPORT_FUNC_STRLEN) != 0)
    return 1;
  ...
  if (expect_event_packet_args("riscv compressed import", &event_frame,
      POLY_TRAP_IMPORT, POLY_MODE_RAW_RISCV, 8, 0,
      import_args_77_99) != 0)
    return 1;
  if (expect_event_packet_import_pc("riscv compressed import",
      &event_frame, POLY_IMPORT_FUNC_STRLEN) != 0)
    return 1;
  ...
  if (expect_event_packet_args("aarch64 opaque import", &event_frame,
      POLY_TRAP_IMPORT, POLY_MODE_RAW_AARCH64,
      NATIVECHECK_IMPORT_UNKNOWN_SELECTOR, 0, import_args_77_99) != 0)
    return 1;
  if (expect_event_packet_import_pc("aarch64 opaque import",
      &event_frame, NATIVECHECK_IMPORT_UNKNOWN_SELECTOR) != 0)
    return 1;
  ...
  if (expect_event_packet_args("riscv opaque import", &event_frame,
      POLY_TRAP_IMPORT, POLY_MODE_RAW_RISCV,
      NATIVECHECK_IMPORT_UNKNOWN_SELECTOR, 0, import_args_77_99) != 0)
    return 1;
  if (expect_event_packet_import_pc("riscv opaque import",
      &event_frame, NATIVECHECK_IMPORT_UNKNOWN_SELECTOR) != 0)
    return 1;
  ...
  if (expect_event_packet_args("riscv-to-aarch64 syscall vector",
      &event_frame, POLY_TRAP_SYSCALL, POLY_MODE_RAW_RISCV, 172, 0,
  ...
  if (expect_event_packet_args("aarch64-to-riscv syscall vector",
      &event_frame, POLY_TRAP_SYSCALL, POLY_MODE_RAW_AARCH64, 172, 7,
  ...
  if (expect_event_packet_args("riscv-to-aarch64 break vector",
      &event_frame, POLY_TRAP_BREAK, POLY_MODE_RAW_RISCV, 5, 0,
      riscv_break_args) != 0)
    return 1;
  ...
  if (expect_event_packet_args("aarch64-to-riscv break vector",
      &event_frame, POLY_TRAP_BREAK, POLY_MODE_RAW_AARCH64, 5, 5,
      aarch64_break_args) != 0)
    return 1;
  ...
  if (expect_event_packet_args("riscv-to-aarch64 import vector",
      &event_frame, POLY_TRAP_IMPORT, POLY_MODE_RAW_RISCV, 8, 0,
  ...
  if (expect_event_packet_args("aarch64-to-riscv import vector",
      &event_frame, POLY_TRAP_IMPORT, POLY_MODE_RAW_AARCH64, 8, 0,
...
static uint64_t nativecheck_direct_pcall_aarch64_import_sum6(uint64_t a0,
  ...
  asm volatile(
    "movq %5, %%r8\n"
    "leaq nativecheck_import_x86_sum6(%%rip), %%r10\n"
    POLY_OP_ENTER_A64
...
int main(void) {
  ...
  if (expect_poly_cpuid != NULL && strcmp(expect_poly_cpuid, "0") == 0) {
    ...
    struct poly_cpuid_regs expected_v2 = poly_cpuid_expected_v2_leaf();
    struct poly_cpuid_regs v2 = poly_read_cpuid(POLY_CPUID_BASE + 10, 0);
    if (v2.eax != expected_v2.eax ||
        v2.ebx != expected_v2.ebx ||
        v2.ecx != expected_v2.ecx ||
        v2.edx != expected_v2.edx) {
      fprintf(stderr,
        "NATIVE_CHECK_FAIL: poly CPUID v2 leaf mismatch eax=0x%x/%x ebx=0x%x/%x ecx=0x%x/%x edx=0x%x/%x\n",
        v2.eax, expected_v2.eax,
        v2.ebx, expected_v2.ebx,
        v2.ecx, expected_v2.ecx,
        v2.edx, expected_v2.edx);
      return 1;
    }
    nativecheck_detail_marker("NATIVE_POLY_CPUID_ARCH_STATE_OK");
