...
Poly lets existing x86_64, AArch64, and RISC-V64 user-mode code share one x86_64 virtual address space. The target is native ABI compatibility with SysV x86_64, AAPCS64, and the RISC-V psABI, not a new compiler-only ABI.
...
- Foreign registers are per-thread XSAVE-style architectural state.
- Recoverable foreign traps produce OS-neutral user-monitor packets.
