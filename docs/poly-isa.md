...
- `0f 24 23 50 4f 4c 59 21`: prototype
  `PCALL.A64.SYSV.HFA3F64RET`, mapping an AAPCS64 homogeneous aggregate
  return in `d0`-`d2` into the x86_64 SysV hidden memory-return buffer.
...
- `0f 24 27 50 4f 4c 59 21`: prototype
  `PCALL.A64.SYSV.HFA3F64ARG`, mapping an x86_64 SysV stack-passed
  three-`double` aggregate argument into AAPCS64 `d0`-`d2`.
- `0f 24 28 50 4f 4c 59 21`: prototype
  `PCALL.A64.SYSV.HFA4F64ARG`, mapping an x86_64 SysV stack-passed
  four-`double` aggregate argument into AAPCS64 `d0`-`d3`.
- `0f 24 29 50 4f 4c 59 21`: prototype
  `PCALL.A64.SYSV.HFA3F32ARG`, mapping an x86_64 SysV `XMM0`/`XMM1`
  three-`float` aggregate argument into AAPCS64 `s0`-`s2`.
- `0f 24 2a 50 4f 4c 59 21`: prototype
  `PCALL.A64.SYSV.HFA4F32ARG`, mapping an x86_64 SysV `XMM0`/`XMM1`
  four-`float` aggregate argument into AAPCS64 `s0`-`s3`.
...
- `0x40000001.ECX`: feature mask.  Bits `0`-`30` mean raw AArch64, raw RISC-V,
  ...
  callers, AArch64 three- and four-lane HFA argument bridges from x86 SysV
...
through `s0`-`s3` into x86_64 SysV `XMM0`/`XMM1`, AArch64 HFA three- and
four-lane `double` and `float` arguments from ordinary x86_64 SysV callers into
`d0`-`d3`/`s0`-`s3`, plus two-`float`
...
`libpolyrootifunc-aarch64.so`) plus compact aggregate, FP64 stack-argument,
and fixed 128-bit vector IFUNC bridge variants
(`aarch64-pcall-cross-root-ifunc-compact-real.so#poly_entry`,
`riscv-pcall-cross-root-ifunc-compact-real.so#poly_entry`,
`aarch64-pcall-cross-root-ifunc-fp64-stack-real.so#poly_entry`,
`riscv-pcall-cross-root-ifunc-fp64-stack-real.so#poly_entry`,
`aarch64-pcall-cross-root-ifunc-vec128-real.so#poly_entry`, and
`riscv-pcall-cross-root-ifunc-vec128-real.so#poly_entry`),
...
`libpolyneededifunc-riscv.so`) plus opposite-ISA compact aggregate, FP64
stack-argument, and fixed 128-bit vector IFUNC bridge pairs
(`aarch64-pcall-cross-ifunc-compact-real.so#poly_entry`,
`riscv-pcall-cross-ifunc-compact-real.so#poly_entry`,
`aarch64-pcall-cross-ifunc-fp64-stack-real.so#poly_entry`,
`riscv-pcall-cross-ifunc-fp64-stack-real.so#poly_entry`,
`aarch64-pcall-cross-ifunc-vec128-real.so#poly_entry`, and
`riscv-pcall-cross-ifunc-vec128-real.so#poly_entry`),
...
`aarch64-pcall-hfa4-f32-real.so#poly_entry`), AArch64 three- and four-lane
homogeneous FP aggregate argument object
(`aarch64-pcall-hfa-arg-real.so#poly_hfa3_f64_arg`,
`#poly_hfa4_f64_arg`, `#poly_hfa3_f32_arg`, and `#poly_hfa4_f32_arg`),
homogeneous float aggregate return objects (`aarch64-pcall-fpair32-real.so#poly_entry` and
...
when bit `0` also requests x86 stack arguments, the first stack qword comes
from AArch64 `x5`, then `x6`, then `x7`, before reading the foreign stack.
...
The `polyexec` and `polyapp` harnesses return neutral breakpoint packet
results.  Real libc-like interop coverage uses descriptor imports instead of
assigning helper semantics to breakpoint numbers.
