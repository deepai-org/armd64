...
Poly keeps x86_64 as the system ISA. AArch64 and RISC-V64 are user-mode
...
- Foreign code fetches from `RIP` in the same virtual address space:
  AArch64 aligned 32-bit, RISC-V64 16/32-bit.
- All frontends share x86_64 TLBs, page permissions, and fault behavior.
- `PENTER`, `PSWITCH`, `PCALL`, and `PIRET` are decoded control flow, not
  `#UD` envelopes.
- Non-x86 architectural state is XSAVE-style state, using prototype component
  `20`.
