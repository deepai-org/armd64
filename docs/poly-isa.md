...
- `0x40000001.ECX`: feature mask.  Bits `0`-`16` mean raw AArch64, raw RISC-V,
  ...
  `{u64,double}`/`{double,u64}` heterogeneous aggregate bridging for native ABI
  `PCALL`.
...
AArch64 additionally has focused bridges for `{u64,double}` and
`{double,u64}` heterogeneous aggregates whose x86 lanes are split across GPR
and XMM registers but whose AAPCS64 lanes live in integer-register bit lanes;
and
...
`riscv-pcall-hetero-rev-real.so#poly_entry`), and scalar double/float FP
