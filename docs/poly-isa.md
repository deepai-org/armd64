...
object and imports a writable root data symbol.  Imported writable data symbols
across the ISA boundary bind directly to the shared virtual address rather than
to a code trampoline.  Cross-ISA TLS, constructor, and IFUNC interposition
remain explicit future compatibility items rather than hidden emulator
behavior.
