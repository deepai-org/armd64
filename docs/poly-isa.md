...
`blr` or RISC-V `jalr` calls to the `strlen` and three-argument `memcmp`
import descriptors inside the neutral callee, preserve the callee's native link
register, and return through the same cross-frontend cookie path.  The
...
aggregate arguments and returns through `XMM0`/`XMM1`, including mixed
...
`riscv-pcall-fpair-real.so#poly_entry`), homogeneous double aggregate argument
...
covers direct descriptor `strlen` and three-argument `memcmp` calls inside
foreign callees, proving descriptor imports are not limited to x86-entered
`PCALL` payloads. The gate also covers
