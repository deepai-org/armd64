...
| Stateful frontend/core composition | `rtl/poly_frontend_stateful_core.sv` feeds `rtl/poly_frontend_core.sv` from architectural frontend/PC state and applies retired commits, decoded raw unconditional direct branch targets, raw interrupt entry/restore updates, and native return-cookie resumes back into `rtl/poly_frontend_state.sv`, while unresolved raw branches hold state until execute-stage target resolution; `rtl/poly_frontend_fpga_top.sv` wraps this stateful core with a unified tagged instruction-memory request/response bus for FPGA integration. |
...
- A directed cycle-budget model exists for the few-cycle `PSWITCH`/`PCALL`
  ...
  (`cells=6634`, `estimated_lcs=3180`), but no place-and-route
