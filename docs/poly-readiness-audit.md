...
| Frontend fetch-to-retire prototype | `rtl/poly_frontend_memory_retire.sv` uses the dual fetch issuer, feeds x86 responses through `rtl/poly_x86_fetch_stage.sv`, feeds raw responses through `rtl/poly_raw_fetch_response_stage.sv`, and connects fetched instruction words to `rtl/poly_frontend_retire.sv` for retire/commit/fault outputs. |
