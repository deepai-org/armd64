...
| Raw instruction classification | `rtl/poly_raw_insn_decode.sv` classifies fetched AArch64/RISC-V loads, stores, atomics, barriers/fences, calls, returns, and traps into hardware sidebands and computes unconditional direct branch/call targets for immediate-control-flow forms while suppressing unresolved conditional branch targets; `rtl/poly_frontend_decode_dispatch.sv` exposes those sidebands, and `rtl/poly_frontend_core.sv` ORs decoded raw memory categories into the existing x86-TSO retirement gate. |
...
| Frontend/transition-stack integration | `rtl/poly_frontend_core.sv` connects precise retired PCALL commits, TSO memory-order backpressure, raw data-memory completion/fault gating, raw interrupt save/restore, trap-packet delivery, ABI signature lookup, CPUID discovery, cycle-budget reporting, execute-resolved raw branch targets, and native return-cookie recovery to `rtl/poly_transition_stack.sv`, blocks PCALL retirement when the transition stack is full or servicing a pop, stalls unresolved raw branch targets until execute resolution, validates resolved raw target canonical/alignment constraints, stalls raw loads/stores/atomics until data-memory completion or fault, and keeps return-cookie recovery from colliding with another same-cycle pop. |
...
- A directed cycle-budget model exists for the few-cycle `PSWITCH`/`PCALL`
  ...
  (`cells=6647`, `estimated_lcs=3075`), but no place-and-route
