...
| Raw instruction classification | `rtl/poly_raw_insn_decode.sv` classifies fetched AArch64/RISC-V loads, stores, atomics, barriers/fences, calls, returns, and traps into hardware sidebands; `rtl/poly_frontend_decode_dispatch.sv` exposes those sidebands, and `rtl/poly_frontend_core.sv` ORs decoded raw memory categories into the existing x86-TSO retirement gate. |
...
- A directed cycle-budget model exists for the few-cycle `PSWITCH`/`PCALL`
  ...
  (`cells=6211`, `estimated_lcs=2859`), but no place-and-route
