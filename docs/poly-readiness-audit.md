...
| FPGA-facing RTL bring-up artifacts | `rtl/poly_ctrl_decode.sv`; `rtl/poly_transition_stack.sv`; `rtl/poly_abi_signature_slots.sv`; `rtl/poly_cpuid_rom.sv`; `make check-poly-rtl` passed. |
...
1. Extend the RTL/FPGA prototype beyond control decode, transition-stack,
   signature-slot, and CPUID primitives to implement raw 32-bit foreign fetch.
