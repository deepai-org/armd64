...
| Frontend/transition-stack integration | `rtl/poly_frontend_core.sv` connects precise retired PCALL commits, TSO memory-order backpressure, raw interrupt save/restore, trap-packet delivery, ABI signature lookup, CPUID discovery, cycle-budget reporting, execute-resolved raw branch targets, and native return-cookie recovery to `rtl/poly_transition_stack.sv`, blocks PCALL retirement when the transition stack is full or servicing a pop, stalls unresolved raw branch targets until execute resolution, validates resolved raw target canonical/alignment constraints, and keeps return-cookie recovery from colliding with another same-cycle pop. |
...
| Stateful frontend/core composition | `rtl/poly_frontend_stateful_core.sv` feeds `rtl/poly_frontend_core.sv` from architectural frontend/PC state and applies retired commits, decoded raw unconditional direct branch targets, execute-resolved raw branch targets, raw interrupt entry/restore updates, and native return-cookie resumes back into `rtl/poly_frontend_state.sv`, while unresolved raw branches hold state until execute-stage target resolution; `rtl/poly_frontend_fpga_top.sv` wraps this stateful core with a unified tagged instruction-memory request/response bus and raw branch-resolution inputs for FPGA integration. |
...
- A directed cycle-budget model exists for the few-cycle `PSWITCH`/`PCALL`
  ...
  (`cells=6644`, `estimated_lcs=3073`), but no place-and-route
