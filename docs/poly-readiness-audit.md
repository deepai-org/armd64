...
| FPGA-facing RTL bring-up artifacts | `rtl/poly_ctrl_decode.sv`; `rtl/poly_transition_stack.sv`; `rtl/poly_abi_signature_slots.sv`; `rtl/poly_cpuid_rom.sv`; `rtl/poly_raw_fetch_plan.sv`; `make check-poly-rtl` passed. |
...
1. Integrate the RTL/FPGA primitives into a frontend/memory prototype that
   performs real instruction fetch, fault ordering, and frontend handoff.
